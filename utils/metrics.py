...
def repeat_score_item(
        train_baskets
        ):
    user_baskets = train_baskets[['user_id', 'date', 'basket_id']].drop_duplicates().sort_values(['user_id', 'date'], ascending=True).groupby(['user_id'])['basket_id'].apply(list).reset_index()
